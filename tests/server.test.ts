/**
 * Iris SYNCD — server integration + unit tests (Vitest).
 *
 *   npm test
 */
...
// Fast stale-device sweep so the heartbeat test observes real eviction cycles quickly
process.env.STALE_CHECK_INTERVAL_MS = '1000';
process.env.STALE_THRESHOLD_MS = '3000';
const {
  server,
  sessions,
  uploads,
...
describe('signaling', () => {
  test('full lifecycle: create, join, upload, playback state machine, end', async () => {
    ...
    const upRes = await fetch(
      `${BASE}/api/sessions/${created.sessionId}/upload?name=unit%20test.mp3`,
      {
        method: 'POST',
    ...
    const startAck = await emitAck(host, 'start-playing', {
      trackId: upJson.trackId,
      duration: 120,
    });
    ...
    const serverTime = await new Promise((res) =>
      dev.timeout(2000).emit('clock-sync', (e, t) => res(e ? null : t)),
    );
    assert.ok(Math.abs(serverTime - Date.now()) < 60_000);

    // PTT relays respect roles
    const pttAnswerP = new Promise((res) =>
      host.once('ptt-answer', ({ deviceId }) => res(deviceId)),
    );
    ...
    assert.equal(
      [...uploads.values()].some((u) => u.id === upJson.trackId),
      false,
    );
  ...
  test('device heartbeat prevents stale eviction', async () => {
    ...
    // Wait long enough for several injected stale sweeps (1s interval, 3s threshold);
    // the 500ms device-ping loop above must keep the device alive through all of them
    await wait(5000);

    const info = await (
      await fetch(`${BASE}/api/sessions/${created.sessionId}`, {
  ...
  test('rejoin-session requires valid host token', async () => {
    ...
    const rejoinOk = await new Promise((res) =>
      host2.emit('rejoin-session', { sessionId, hostToken }, res),
    );
    ...
    const rejoinFail = await new Promise((res) =>
      host3.emit('rejoin-session', { sessionId, hostToken: 'wrong-token' }, res),
    );
    ...
    const rejoinNoToken = await new Promise((res) =>
      host4.emit('rejoin-session', { sessionId }, res),
    );
